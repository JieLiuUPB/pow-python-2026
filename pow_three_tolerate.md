...
- `FastCollusionSimulation(CollusionSimulation)`：
  ...
  - 覆盖 `simulate_one_run`，在事件循环中增加进度输出（默认每 10% 输出一次）。
- `build_arg_parser()`：
...
补充：当 `--jobs > 1` 时，使用 `ProcessPoolExecutor` 按 run 维度并行执行，属于多核模式。

## 4. 进度输出机制（新增）
- 参数：`--progress-step-percent`（默认 `10`）。
- 含义：按 `target_blocks_long` 的百分比阈值输出。
- 示例：目标 `2,016,000`，步长 `10` 时，会在约 10%、20%、...、100% 达到时打印一次：
  - `[run 1] progress: 10% (xxxxx/2016000 canonical blocks)`
- 实现位置：`FastCollusionSimulation._build_progress_targets` + `_emit_progress` + 覆盖的 `simulate_one_run`。
...
- `--progress-step-percent`：进度输出步长百分比，默认 `10`。
- `--jobs`：并行进程数，默认 `1`（串行）。大于 1 时按 run 并行（多核）。
