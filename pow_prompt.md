...
---
下面是在你现有仿真规格文档基础上的**增补内容**，用于完成 PDF Section 6 里“orphan block ratio 随 (p) 变化”的实验验证。你可以把这段直接追加到原 Markdown 文档中（建议放在“分组 5 指标统计”之后，“分组 7 绘图规范”之前）。

---

## 分组 5.5：Orphan Block Ratio（用于验证 Section 6 的检测信号）

### 5.5.1 定义与统计口径（必须固定，否则曲线不可比）

**统计对象范围：已发布（publicly visible）的块。**
因为“攻击者私藏但最终放弃的私有块”并不会进入网络观测，不能算 orphan；Section 6 的检测也应基于网络可观测数据。

对任意一次 run，定义：

* `published_blocks`：本次仿真期间所有“进入网络”的块集合

  * 包含：诚实矿池挖到即发布的块
  * 包含：攻击者在 deadline 或成功时公开的块（包括一次性公开的 `n` 与 `n+1` 两块）
  * **不包含**：攻击者在放弃条件下丢弃的私有块（未发布）
* `canonical_blocks`：最终 canonical 链上的块集合（D1）
* `orphan_blocks = published_blocks - canonical_blocks`

定义 **orphan block ratio（孤块率）**：

[
R_{\text{orphan}}=\frac{|orphan_blocks|}{|published_blocks|}
]

> 说明：该定义与“网络观测到的孤块比例”一致，也更贴近 Section 6 的“检测信号”。
> 这也避免了把“未发布的私有块”算作孤块而人为抬高比例。

此外可选输出两个分解指标，便于解释曲线形状（建议保留）：

* `R_orphan_A = A_orphan_published / A_published_total`
* `R_orphan_H = H_orphan_published / H_published_total`

其中：

* `A_published_total`：A 公开发布到网络的块总数
* `H_published_total`：H 发布到网络的块总数（即 H 挖出的总块数，因为它全公开）

### 5.5.2 run 输出新增字段（写入 raw_runs.csv）

在 `raw_runs.csv` 中新增以下字段（每行 = 一个 run）：
...
* `orphan_A_published`
* `orphan_H_published`
* `orphan_ratio = orphan_total / published_total`
* `orphan_ratio_A = orphan_A_published / published_A_total`（若分母为 0 则写 NaN）
* `orphan_ratio_H = orphan_H_published / published_H_total`

### 5.5.3 聚合 summary 新增表（用于画折线图）

建议为 Section 6 单独生成一份聚合表：

`orphan_ratio_summary.csv`（按 scenario 和 p 聚合）

字段：

* `scenario`（建议用场景 1 和场景 2 都画一遍；场景 1 更“固定高度可比”，场景 2 更像真实“固定时间观测”）
* `p`
* `orphan_ratio_mean`
* `orphan_ratio_std`
* `orphan_ratio_max_over_runs`（10 次 run 中的最大值，用于回答“最大能到多少”）
* `runs=10`

> 你要求“让人清晰看出最大的 orphan block ratio 能达到多少”。
> 因此除了均值曲线，必须补充 `max_over_runs`（或至少在图上标注最大点）。

### 5.5.4 绘图建议（Section 6 主图 + 辅助标注）

**主图（推荐）**：折线图（带误差棒）

* x：(p)
* y：`orphan_ratio_mean`
* 误差棒：`± orphan_ratio_std`
* 在每个 (p) 上可用小标记（或同图第二条线）表示 `orphan_ratio_max_over_runs`

**最大值呈现方式（二选一）**：

* 方式 A：同图增加一条虚线/细线表示 `max_over_runs`
* 方式 B：在图右上角加文本标注：
  `max orphan ratio across all p and runs = ... (at p=...)`

**可选的解释图（如果你想更像论文）**：

* 图：`p` vs `orphan_ratio_A_mean` 与 `orphan_ratio_H_mean` 两条线
  用于说明孤块主要由哪一方造成（通常 TBW 会增加分叉与竞争，从而推动孤块上升）。

### 5.5.5 与“最大可达多少”的严格定义

你需要在正文里用一句话固定“最大”的定义，否则审稿人会问“最大是均值最大还是单次最大”：

* **最大均值**：(\max_p \mathbb{E}[R_{\text{orphan}}|p])（折线图最高点）
* **最大单次观测**：(\max_{p,run} R_{\text{orphan}}(p,run))（10 次 run 中的最大值，建议报告这个更保守/更直观）

你的需求更像后者，所以建议在图注里写清楚：
“曲线为 10 次独立仿真均值；点线/标注为同一 (p) 下 10 次仿真的最大孤块率”。

---

## 分组 6（数据保存）补充

在每个 scenario 的输出目录中追加：

* `orphan_ratio_summary.csv`

并在 `summary.csv` 中也可附带：

* `orphan_ratio_mean`, `orphan_ratio_std`

**功能清单**

* [未完成] 在仿真引擎中维护 `published_set`（所有发布块 id）与 `published_by_miner` 计数器。
* [未完成] 在 run 结束时由 `orphan_blocks = published_set - canonical_set` 计算孤块集合与比例。
* [未完成] 将 orphan 相关字段写入 `raw_runs.csv`。
* [未完成] 生成 `orphan_ratio_summary.csv`（按 scenario,p 聚合 mean/std/max）。
* [未完成] 绘制 Section 6 的孤块率折线图（均值+误差棒+最大值标注）。

---

## 分组 7（绘图规范）补充：Section 6 孤块率图

新增图：`s6_orphan_ratio_vs_p.png`
...
* 对场景 1（固定 2016 主链块）：
  ...
  * y：mean(orphan_ratio)
  * errorbar：std
  * overlay：max_over_runs 或 text 标注全局 max
* （可选）对场景 2（固定时间）同样画一张，以展示“时间观测口径”下孤块率是否更高/更稳定。

**功能清单**

* [未完成] 实现 `plot_orphan_ratio_vs_p(summary_df)`，保存图片并导出对应数据 CSV。
